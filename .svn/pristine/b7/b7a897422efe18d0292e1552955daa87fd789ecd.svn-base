...
<body>
	...
	<div class="news-center">
		<div class="news-left">
			<a class="news-leftimg"> <img src="img/news-left.jpg"
				style="position:relative" /> <!-- <span class="gsxw">公司新闻</span> <span class="hyzx">行业资讯</span> -->
				<span
				style="position:absolute; top:30px; left:50px; color:#fff; font-size:23px">公司新闻</span>
				<p
					style="position:absolute; top:45px; left:50px; color:#fff; font-size:16px">Press
					center</p>
			</a>
		</div>
					...
					.post(
							...
							function(json) {
								...
								$json
										.each(function() {
											html = html
													+ '<a href="news/newsInfo.html?id='
													+ this.id
													+ '"><li><span><img src="img/dian.jpg" /><i>'
													+ this.title
													+ '</i> <i style=" float:right; font-size:14px; color:#999;text-align:right;">'
													+ this.createtime
													+ '</i></span></li></a>';
										});
								$("#content").html(html);
	...
	<div class="fanye">
		<input type="hidden" id="pages" value="1"> <a><span
			class="fanye-but" onclick="findByPage(1)">首页</span><span
			class="fanye-but" onclick="findByPage(2)">上一页</span><span
			class="fanye-but" onclick="findByPage(3)">下一页</span><span>共<i
				id="total">120</i>条记录
		</span></a>
	</div>
	<div class="footer">
